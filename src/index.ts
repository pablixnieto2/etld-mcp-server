...
class EtldMcpServer {
  ...
  private openapi: OpenAPISchema | null = null;
  ...
  private async fetchOpenApi(): Promise<OpenAPISchema> {
  ...
  private resolveSchema(schema: any, components: any): any {
    ...
    if (schema.anyOf) {
        return { ...schema, anyOf: schema.anyOf.map((s: any) => this.resolveSchema(s, components)) };
    }

    if (schema.allOf) {
        return { ...schema, allOf: schema.allOf.map((s: any) => this.resolveSchema(s, components)) };
    }

    if (schema.oneOf) {
        return { ...schema, oneOf: schema.oneOf.map((s: any) => this.resolveSchema(s, components)) };
    }
    ...
    this.server.setRequestHandler(ListToolsRequestSchema, async () => {
      ...
      for (const [pathStr, methods] of Object.entries(this.openapi.paths)) {
        ...
        if (methods.post) {
          ...
          if (pathStr.startsWith("/health") || pathStr.startsWith("/v1/auth")) continue;
